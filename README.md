...
| Tinta de hover/seleção | sempre o translúcido `--surface-muted-hover` (v0.7.3), a 12% no claro e 10% no escuro (v0.7.6) | Um tom sólido não sabe sobre o que está empilhado. O realce de item de menu era `bg-surface-elevated` e ficava **invisível no tema claro**, onde painel e elevated são ambos `#FFFFFF`. As opacidades diferem porque tinta escura sobre fundo claro rende menos degrau percebido: a 9% o claro dava ΔL\* 7,1 contra 9,2 do escuro |
